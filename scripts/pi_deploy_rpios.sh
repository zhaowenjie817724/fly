#!/bin/bash
# ==========================================================
#  通感之眼2.0 - Raspberry Pi OS (Legacy, 64-bit) Full 部署脚本
#  适用: Raspberry Pi 5 (2GB RAM + 32GB SD)
#  系统: Raspberry Pi OS (Legacy) Full (Bullseye / Debian 11)
#  用法: chmod +x pi_deploy_rpios.sh && ./pi_deploy_rpios.sh [用户名] [用户组] [fly目录]
# ==========================================================
...
echo "=========================================="
echo "  通感之眼2.0 - 树莓派部署"
echo "  系统: Raspberry Pi OS (Legacy) Full"
echo "  硬件: Raspberry Pi 5 (2GB)"
echo "=========================================="
...
if [ -f /etc/os-release ]; then
    ...
    if [[ "${ID:-}" != "raspbian" && "${ID:-}" != "debian" ]]; then
        log_warn "此脚本面向 Raspberry Pi OS; 当前 ID=${ID:-unknown}"
    fi
else
    log_warn "无法检测操作系统版本"
fi
...
# 检查磁盘空间
DISK_FREE=$(df -BG "$FLY_DIR" | tail -1 | awk '{print $4}' | tr -d 'G')
...
sudo apt-get install -y -qq \
    ...
    python3-libcamera \
    python3-picamera2 \
    2>/dev/null || log_warn "libcamera/picamera2 安装失败，将使用 v4l2 接口"
...
if [ "$CURRENT_SWAP" -lt 1500 ]; then
    log_info "当前 Swap: ${CURRENT_SWAP}MB，扩展到 ${SWAP_SIZE}MB..."
    ...
    if [ -f /etc/dphys-swapfile ]; then
        ...
        log_ok "Swap 已扩展到 ${SWAP_SIZE}MB (dphys-swapfile)"
    else
        if [ ! -f /swapfile_wurenji ]; then
            sudo fallocate -l ${SWAP_SIZE}M /swapfile_wurenji
        ...
        log_ok "Swap 文件已创建 (${SWAP_SIZE}MB)"
    fi
else
...
if [ -f "$FLY_DIR/scripts/pi/setup-zram.sh" ]; then
    bash "$FLY_DIR/scripts/pi/setup-zram.sh" || log_warn "zram 配置失败"
else
...
if [ "$CAM_DETECTED" = false ]; then
    ...
    log_warn "CSI: 检查 raspi-config → Interface → Camera"
fi
...
if [ "$FC_DETECTED" = false ]; then
    log_warn "未检测到飞控设备"
    log_warn "USB飞控: 插入Pixhawk后检查 /dev/ttyACM0"
    log_warn "UART飞控: 检查 /dev/serial0 (需在 raspi-config 启用)"
fi
...
echo ""
echo "=========================================="
echo -e "  ${GREEN}Raspberry Pi OS 部署完成!${NC}"
echo "=========================================="
echo ""
echo "  启动服务 (推荐):"
...
if [ -n "$FC_PORT" ]; then
    echo "  飞控连接 ($FC_PORT):"
    echo "    编辑 configs/pi_bookworm.yaml → telemetry.mavlink.serial_port = \"$FC_PORT\""
    echo ""
fi
echo "  系统优化:"
